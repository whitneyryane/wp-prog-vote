...
class Prog_Vote_Admin {
	...
	* @since    1.0.0
	*/
	public function redistribute( $r, $el = false ) {
		...
		if ( $el ){
			...
			echo 'TR: '.$key.' => '.$val.'<br />';
				if ( $val > $r['th'] ) {
				echo 'TH: '.$val.' > '.$r['th'].'?<br />';					
					if ( in_array( $key, $r['wc'] ) ) { } else {
					echo 'WC: '.$key.' is not in array.<br />';
						array_push( $r['wc'], $key );
						echo 'WC: ';var_dump($r['wc']);echo '<br />';
						$can = array_search( $key, $r['pc'] );
						
						unset( $r['pc'][$can] );
						echo 'PC: ';var_dump($r['pc']);echo '<br />';
						$cr = 1 - ( $r['th'] / $val );
						echo 'CR: '.$cr.'<br />';
						foreach ( $r['rm'][$key] as $ke => $va ) {
							
							$rmArray = explode( ' ', $ke );
							echo 'RMaRRAY: ';var_dump($rmArray);echo '<br />';
							$ind = array_search( $key, $rmArray );
							echo 'ind: '.$ind.' => '.$rmArray[$ind].'<br />';
							for ( $i = $ind + 1; $i > 0; $i++ ) {
								...
								if ( isset( $rmArray[$i] ) ) {
									...
									if ( in_array( $rmArray[$i], $r['pc'] ) ) {
										...
										echo 'RM: ';var_dump($r['rm']);echo '<br /><br />';
										echo 'RD: ';var_dump($r['rd']);echo '<br /><br />';
										echo 'TR: ';var_dump($r['tr']);echo '<br /><br />';
		...
		if ( count( $r['wc'] ) < $r['nw'] ) {
			...
			if ( $r['i'] > 1 ) { 
				...
				foreach ( $v as $key => $val ) {	
					...
					$r['rm'][$k[0]][$key] = $val;
					echo 'RM-'.$k[0].': ';var_dump($r['rm'][$k[0]]);echo '<br />';
	...
	*/
	public function tab_election(){
		...
		if ( get_post_meta( $args['eid'], 'tabulated', true ) ) {
			...
			foreach( $args['r'] as $r ) { echo 'Race: '.get_the_title($r->post_id).'<hr />';
